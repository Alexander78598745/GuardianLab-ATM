// --- CONFIGURACIÓN BASE DE DATOS (NO LÍMITES) ---
...
document.addEventListener('DOMContentLoaded', async () => {
    await initDB(); // Iniciar BD Gigante
    cargarPorteros();
    cargarHistorialReportes(); 
    cargarPartidosHistorial();
    
    const today = new Date().toISOString().split('T')[0];
    const fSes = document.getElementById('fechaSesion'); if(fSes) fSes.value=today; // Si existe
...
async function cargarPorteros(){ 
    ...
    if(document.getElementById('eval-portero')) document.getElementById('eval-portero').innerHTML = opts; 
    
    // FIX LIVE MODAL DROPDOWN
    if(document.getElementById('conf-portero-titular')) {
        // Se llenará al elegir equipo, pero necesitamos la base
    }
}
...
function procesarPortero(){ 
    ...
    const save = async (foto) => { 
        ...
        // Si edita, mantenemos ID, si no, nuevo.
    ...
    if(file){
        ...
        // Si editamos y no hay foto nueva, buscar la vieja
...
async function guardarReporteCompleto() {
    ...
    // 1. GUARDAR REPORTE PACK
    const nuevoReporte = {
        id: Date.now(),
        porteroId: parseInt(pid),
        fecha: fecha,
        acciones: evaluacionesTemporales 
    };
    await dbSave('reportes', nuevoReporte);

    // 2. GUARDAR DATOS DESGLOSADOS
...
async function cargarHistorialReportes() {
    const reportes = await dbGetAll('reportes');
    const ps = await dbGetAll('porteros');
...
async function actualizarGrafica() {
    ...
    if(!pid) return;
    const lista = await dbGetAll('seguimientos');
...
function iniciarLivePro() { 
    ...
    partidoLive = { config:cfg, acciones:[], marcador:{local:0,rival:0}, porterosJugaron:new Set([cfg.titular]), minutosJugados:{}, porteroActualId:cfg.titular, parteActual:'Pre-Partido', crono:{seg:0,int:null,run:false} }; 
...
async function confirmarCambio(){ const pid = parseInt(document.getElementById('select-cambio-portero').value); const ps = await dbGetAll('porteros'); const entra = ps.find(p=>p.id===pid); const sale = ps.find(p=>p.id===partidoLive.porteroActualId); partidoLive.porteroActualId = pid; partidoLive.porterosJugaron.add(pid); if (!partidoLive.minutosJugados[pid]) partidoLive.minutosJugados[pid] = 0; regEv('CAMBIO', `Entra ${entra.nombre}, Sale ${sale.nombre}`); actualizarUI(); document.getElementById('modal-cambio').style.display='none'; }
