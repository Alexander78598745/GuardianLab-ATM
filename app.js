...
// --- PORTEROS (CON COMPRESOR DE IMAGEN PARA TABLET) ---
...
function cargarPorteros(){ 
    const l=JSON.parse(localStorage.getItem('guardian_porteros'))||[]; 
...
function procesarPortero(){ 
    ...
    const save=(foto)=>{ 
        ...
        try {
            ...
            alert("⚠️ Memoria llena. Intenta borrar porteros antiguos o usar fotos más pequeñas.");
        }
    }; 
    
    if(file){
        // COMPRESOR DE IMAGEN PARA TABLET
        ...
        r.onload=(e)=>{
            ...
            img.onload = () => {
                ...
                // Redimensionar a máximo 300px (Suficiente para miniatura)
                const max = 300;
                let w = img.width; let h = img.height;
                if(w>h){ if(w>max){ h*=max/w; w=max; } } else { if(h>max){ w*=max/h; h=max; } }
                canvas.width = w; canvas.height = h;
                ctx.drawImage(img, 0, 0, w, h);
                // Guardar comprimido (calidad 0.7)
