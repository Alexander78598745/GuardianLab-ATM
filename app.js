...
// HELPERS DB (Adaptados a Firebase)
function dbSave(storeName, data) {
    // Usamos el 'id' del dato como ID del documento en Firebase para poder editarlo luego
    return db.collection(storeName).doc(String(data.id)).set(data)
        .then(() => true)
        .catch(err => { console.error("Error saving", err); return false; });
}

function dbGetAll(storeName) {
    return db.collection(storeName).get().then(querySnapshot => {
        const list = [];
...
let partidoEnEdicion = null; // Para edición posterior
...
document.addEventListener('DOMContentLoaded', async () => {
    // Ya no hay initDB local, Firebase ya está init arriba
    
    // Configurar listener en tiempo real para porteros
    db.collection("porteros").onSnapshot((snapshot) => {
        const l = [];
        snapshot.forEach((doc) => {
             l.push(doc.data());
        });
        actualizarListaPorterosUI(l);
    });

    // Configurar listener para partidos
    db.collection("partidos").orderBy("id", "desc").onSnapshot((snapshot) => {
        cargarPartidosHistorialDesdeSnap(snapshot);
    });
    
    // Configurar listener para reportes
    db.collection("reportes").orderBy("id", "desc").onSnapshot((snapshot) => {
        cargarHistorialReportesDesdeSnap(snapshot);
    });
...
window.onbeforeunload = function() {
    if (partidoLive.crono.run || partidoLive.parteActual === '1ª Parte' || partidoLive.parteActual === '2ª Parte') {
        return "Hay un partido en curso. ¿Seguro que quieres salir?";
    }
};
...
function cambiarSeccion(sec) { 
    ...
    // Si entramos en datos, actualizar grafica
    if(sec === 'datos') {
        // Nada extra, ya se carga al elegir portero
    }
}

// --- PORTEROS (DB + COMPRESOR) ---
...
function actualizarListaPorterosUI(l) {
    ...
    if(file){
        ...
        if(porteroEnEdicionId) {
            dbGetAll('porteros').then(list => {
                const old = list.find(x=>x.id == porteroEnEdicionId); // == para coincidir tipo
...
async function cargarDatosEdicion(id){
    ...
    if(p){ 
        ...
        porteroEnEdicionId=p.id; // Asegurar ID original
...
function borrarPortero(id){if(confirm("¿Borrar?")){ dbDelete('porteros', id); }}
...
async function guardarReporteCompleto() {
    ...
    const nuevoReporte = { id: Date.now(), porteroId: parseInt(pid), fecha: fecha, acciones: evaluacionesTemporales };
    await dbSave('reportes', nuevoReporte);

    // Guardar para estadisticas
    evaluacionesTemporales.forEach(async item => {
        await dbSave('seguimientos', {
...
async function generarPDFReporteLote(reporte) {
...
function cargarHistorialReportesDesdeSnap(snapshot) {
    const cont = document.getElementById('lista-seguimientos');
    if(!cont) return;
    cont.innerHTML = '';
    
    // Convertir snap a lista y obtener IDs porteros
    let reportes = [];
    snapshot.forEach(doc => reportes.push(doc.data()));
    
    // Necesitamos porteros para los nombres
    dbGetAll('porteros').then(ps => {
        reportes.forEach(rep => {
            const p = ps.find(x => x.id == rep.porteroId);
            if(!p) return;
            cont.innerHTML += `<div class="eval-card" style="border-left: 5px solid var(--atm-blue)"><div><div style="font-weight:bold;font-size:0.9rem;">${p.nombre}</div><div style="font-size:0.8rem;color:var(--text-sec);">Reporte Completo (${rep.acciones.length} acciones)</div><div style="font-size:0.75rem;margin-top:4px;">${rep.fecha}</div></div><div style="display:flex; gap:5px;"><button class="btn-icon-action" onclick="verPDFReporte(${rep.id})" title="Ver PDF">📄</button><button class="btn-trash" onclick="borrarReporte(${rep.id})">🗑️</button></div></div>`;
        });
    });
}

async function verPDFReporte(id) {
    const doc = await db.collection('reportes').doc(String(id)).get();
    if(doc.exists) generarPDFReporteLote(doc.data());
}

function borrarReporte(id) { if(confirm("¿Borrar?")) { dbDelete('reportes', id); } }
...
async function actualizarGrafica() {
    ...
    const snap = await db.collection('seguimientos').where('porteroId', '==', parseInt(pid)).get();
    ...
    window.myChart = new Chart(ctx, { type: 'line', data: { labels: labels, datasets: [{ label: 'Puntaje', data: data, borderColor: '#CB3524', backgroundColor: 'rgba(203,53,36,0.1)', tension: 0.4, fill: true }] }, options: { scales: { y: { min: 0, max: 5 } }, plugins: { legend: { display: false } } } });
}

// --- LIVE MATCH ---
async function abrirConfigPartido() { 
    if(partidoLive.parteActual !== 'Pre-Partido' && partidoLive.parteActual !== 'Final') { if(!confirm("⚠️ ¡HAY UN PARTIDO EN JUEGO!")) return; } 
    const ps = await dbGetAll('porteros');
    const equipos=[...new Set(ps.map(p=>p.equipo))].sort(); 
    const selE=document.getElementById('conf-equipo'); selE.innerHTML='<option value="">Equipo ATM...</option>'; 
    equipos.forEach(e=>selE.innerHTML+=`<option value="${e}">${e}</option>`); 
    document.getElementById('modal-config-partido').style.display='flex'; 
}

async function filtrarPorterosPorEquipo() { 
    const eq = document.getElementById('conf-equipo').value; 
    const ps = await dbGetAll('porteros'); 
    const sel=document.getElementById('conf-portero-titular'); sel.innerHTML='<option value="">Titular...</option>'; 
    ps.filter(p=>p.equipo===eq).forEach(p=>sel.innerHTML+=`<option value="${p.id}">${p.nombre}</option>`); 
}

function iniciarLivePro() { 
    const cfg = { 
        equipo: document.getElementById('conf-equipo').value, 
        tipo: document.getElementById('conf-tipo').value, 
        titular: parseInt(document.getElementById('conf-portero-titular').value), 
        rival: document.getElementById('conf-rival').value, 
    ...
    if(!cfg.equipo||!cfg.titular||!cfg.rival) return alert("Faltan datos"); 
    
    // Limpiar backup viejo si iniciamos nuevo
...
// --- PERSISTENCIA Y CRONO REAL ---
function guardarEstadoLive() {
    // Convertir Set a Array para JSON
    const estado = { ...partidoLive, porterosJugaron: [...partidoLive.porterosJugaron] };
    localStorage.setItem('guardian_live_backup', JSON.stringify(estado));
}

function recuperarPartidoEnCurso() {
    const backup = localStorage.getItem('guardian_live_backup');
    if (backup) {
        if (confirm("⚠️ Se detectó un partido en curso. ¿Deseas recuperarlo?")) {
            const estado = JSON.parse(backup);
            estado.porterosJugaron = new Set(estado.porterosJugaron);
            partidoLive = estado;
            
            cambiarSeccion('live');
            document.getElementById('live-equipo-local').innerText = partidoLive.config.equipo;
            document.getElementById('live-equipo-rival').innerText = partidoLive.config.rival;
            ...
            if (partidoLive.crono.run) {
                partidoLive.crono.int = setInterval(() => {
                    const now = Date.now();
                    const diff = Math.floor((now - partidoLive.crono.startTs) / 1000);
                    partidoLive.crono.seg = partidoLive.crono.savedSeg + diff;
                    updCrono();
                    // Recuperar minutos jugados real time
                     if(partidoLive.porteroActualId) {
            ...
            if(partidoLive.parteActual === 'Pre-Partido') document.getElementById('btn-start-partido').style.display='block';
            else if(partidoLive.parteActual === '1ª Parte') document.getElementById('btn-fin-1').style.display='block';
            else if(partidoLive.parteActual === 'Descanso') document.getElementById('btn-ini-2').style.display='block';
            else if(partidoLive.parteActual === '2ª Parte') document.getElementById('btn-fin-partido').style.display='block';
...
async function regEv(tipo, nom, res=null, obs=null){ 
    const min=Math.floor(partidoLive.crono.seg/60)+1; 
    const ps = await dbGetAll('porteros'); 
    const p=ps.find(x=>x.id==partidoLive.porteroActualId); 
    const ev={id:Date.now(), min:min+"'", parte:partidoLive.parteActual, seg:partidoLive.crono.seg, tipo, nom, pid:partidoLive.porteroActualId, pnom:p?p.nombre:'-', res, obs}; 
...
async function confirmarCambio(){ 
    ...
    regEv('CAMBIO', `Entra ${entra.nombre}, Sale ${sale.nombre}`); actualizarUI(); document.getElementById('modal-cambio').style.display='none'; guardarEstadoLive(); 
}
...
async function prepararVistaPreviaPDF() {
    ...
    // Capturar datos del análisis del DOM
    let analisisData = {};
    ...
    // Guardar partido con datos RAW para editar
    const partidoGuardado = {
        ...
        raw: { ...partidoLive, porterosJugaron: [...partidoLive.porterosJugaron], analisis: analisisData } 
    };
...
async function verPDFHistorial(id){
    const doc = await db.collection('partidos').doc(String(id)).get();
    if(doc.exists){
         const match = doc.data();
         document.getElementById('preview-content').innerHTML = match.htmlData; 
         document.getElementById('printable-area').innerHTML = match.htmlData; 
         document.getElementById('modal-pdf-preview').style.display = 'flex'; 
    }
}
function borrarHistorial(id){if(confirm("¿Borrar?")){ dbDelete('partidos', id); }}

// --- EDICIÓN DE ANÁLISIS ---
async function abrirEdicionAnalisis(id) {
    const doc = await db.collection('partidos').doc(String(id)).get();
    if (!doc.exists) return alert("Este partido no existe.");
    const match = doc.data();
    ...
    match.raw.porterosJugaron.forEach(pid => {
        ...
        container.innerHTML += `
            <div class="pdf-obs-box">
                ...
                <textarea id="edit_pos_${pid}" class="pdf-input-read" placeholder="Lo POSITIVO...">${prev.pos}</textarea>
                <textarea id="edit_neg_${pid}" class="pdf-input-read" placeholder="Lo NEGATIVO...">${prev.neg}</textarea>
                <textarea id="edit_tras_${pid}" class="pdf-input-read" placeholder="Trascendencia...">${prev.tras}</textarea>
            </div>
        `;
    });
